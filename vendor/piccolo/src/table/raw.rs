    ...
    pub fn set(
        ...
        Ok(if value.is_nil() {
            ...
            if optimal_size > old_array_size {
                ...
                //
                // SLOTTED PATCH (see ../../VENDOR.md): drop every dead key first. A removed key
                // leaves a tombstone whose `live_key()` is `None`, and the rehash below would
                // panic on it. This is the fix upstream shipped after 0.3.3 in
                // `RawTable::reserve_map`; without it a Lua script that does `t[k] = nil` and then
                // grows the same table aborts the process.
