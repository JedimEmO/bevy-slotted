// SLOTTED PATCH (see ../VENDOR.md): a crates.io dependency is built with
// `--cap-lints allow`; a vendored path dependency is treated as local code and
// is not. These three fire on unmodified upstream 0.3.3 under current rustc
// and would otherwise print on every build of the workspace. They are style
// lints only, allowed rather than fixed so the vendored source stays as close
// to upstream as possible.
#![allow(irrefutable_let_patterns, mismatched_lifetime_syntaxes)]
