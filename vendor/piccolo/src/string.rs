            ...
            fn drop(&mut self) {
                match self.header.buffer {
                    Buffer::Indirect(ptr) => unsafe {
                        // SLOTTED PATCH (see ../VENDOR.md): `(*ptr).len()`
                        // autorefs through a raw pointer, which is a hard
                        // error on current rustc. A vendored crate does not
                        // get `--cap-lints allow`, so it has to be spelled
                        // with the raw-slice `len`, which reads the metadata
