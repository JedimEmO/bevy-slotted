...
slotted is fifteen crates. The shape is deliberate: the rules of an inventory
...
**Every boundary with two implementations is a port.** There are four:
...
4. `SlottedEcsSet::Reconcile` polls the authority for acks and resyncs. A resync
   overwrites local state; prediction is a guess, not a decision.
