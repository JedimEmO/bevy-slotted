...
Status: v1.0, 2026-09-05. Bevy 0.19.1, mlua 0.11 (Luau, vendored). Companion to `docs/PLAN.md`
...
|---|---|---|---|
| `RegisterItem` | `id: String, def: Value` | Data | `ItemDef` via `Value -> ron::Value -> into_rust`, `items.replace` |
...
| `RegisterWidget` | `id, def` | Data | `WidgetDef` payload: a `UiNodeDef` template; B registers a `TemplateWidget` per entry in `WidgetRegistry` whose `Anchor { id: "children" }` receives the caller's children. No params in Phase 4. |
...
`slotted_model::Value` derives an externally tagged serde form, which Lua cannot produce. Every
`Value` field above is `#[serde(with = "slotted_script::value::untagged")]` (a `Vec<Value>` uses
...
an error); `null`/nil -> error (there is no `Value::Null`, omit the field). Adapter rules for
...
gate ADR 0001 requires. Minimum cases A ships: each `register_*`, id namespacing, `inject`,
...
ModManifest)>)` is the in-memory constructor for tests. `PackLayout { base: PathBuf, mods: ModSet,
...
2. **Data**: `Registries::new()`; `DataStage::new(order).load_into(&LayeredSource, &mut regs)`;
   ...
   registrations, records `ModError::Script`, and keeps its RON.
...
1. Re-run **Data** for the whole set (registries are one interned space; a per-mod incremental
   ...
   **previous** `Registries`, emit `ModFailed`, and stop here.
...
`copper_chest.item.copper_chest`, and unqualified keys are looked up as written. `Locales::resolve
